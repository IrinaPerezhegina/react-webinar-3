...
// При смене языка изменяются данные сервиса АПИ ,так кажется быстрее, нет запутанности в коде, функция изменяет данные АПИ, откуда берутся все запросы на сервер
  // Легче с помощью хука useTranslate получать данные о языке(lang) и при смене языка на странице обновлять запросы на сервер и получать данные на измененном языке
setLang(lang) {
